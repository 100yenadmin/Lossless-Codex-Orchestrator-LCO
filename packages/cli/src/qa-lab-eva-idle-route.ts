...
export async function runEvaIdleRoute(options: EvaIdleRouteOptions): Promise<EvaIdleRouteReport> {
  ...
  const report: EvaIdleRouteReport = {
    ...
    nextSafeCommands: [
      `cd <reviewed-checkout> && npm run build && LCO_CODEX_TRANSPORT=daemon node ./dist/packages/cli/src/index.js qa-lab eva-idle-route --evidence-dir <path> --mcp-bin <exact-package-bin> --package-tarball <canonical-1.7.0.tgz> --package-version ${options.packageVersion} --candidate-sha ${EVA_IDLE_ROUTE_CANDIDATE_SHA} [--execute] [--strict]`
    ]
  };
  try {
