...
function binaryProbeDiagnosticCommands(
  ...
  if (diagnostic.classification === "not_provided") {
    return [
      `${tarballLookup} --json`,
      `${tarballExtractPrefix} && version="$(node "$tmp_dir/package/dist/packages/cli/src/index.js" --version)" && package_version="$(node -pe "require(process.argv[1]).version" "$tmp_dir/package/package.json")" && test -n "$version" && test -n "$package_version" && test "$version" = "$package_version" && ${binaryProbeJsonWriteCommand(packageVersion)}`,
...
function binaryProbeJsonWriteCommand(packageVersion: string): string {
  const writer = [
    ...
    "fs.writeFileSync('binary-probe.json', JSON.stringify({ kind: 'loo_published_binary_probe_evidence', publicSafe: true, rawSecretIncluded: false, expectedVersion, observedVersion, resolvedBinarySource: 'package_tarball', pathShadowed: false, tarballBinaryVersion, packageJsonVersion }) + '\\n');"
  ].join(" ");
