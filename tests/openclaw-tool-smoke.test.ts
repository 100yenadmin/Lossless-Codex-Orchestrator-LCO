...
function createFakeOpenClaw(
  ...
  options: {
    ...
    mismatchedSuccessfulPostCreateNestedRef?: boolean;
    mismatchedSuccessfulPostCreateIndexRef?: boolean;
    inconsistentSuccessfulPostCreateActionAlias?: boolean;
...
test("OpenClaw tool smoke accepts a camelCase successful post-create proof", () => {
  const dir = mkdtempSync(join(tmpdir(), "loo-openclaw-tool-smoke-post-create-camel-case-"));
  const toolName = "loo_codex_start_thread_post_create_proof";
  const { bin, callsPath } = createFakeOpenClaw(dir, [toolName], "flat", {
    successfulPostCreateProof: true,
    camelCaseSuccessfulPostCreateProof: true
  });
  const previous = process.env.OPENCLAW_FAKE_CALLS;
  process.env.OPENCLAW_FAKE_CALLS = callsPath;
  try {
...
test("OpenClaw tool smoke rejects post-create proof whose nested refs target another thread", () => {
  const dir = mkdtempSync(join(tmpdir(), "loo-openclaw-tool-smoke-post-create-mismatched-ref-"));
  const toolName = "loo_codex_start_thread_post_create_proof";
  const { bin, callsPath } = createFakeOpenClaw(dir, [toolName], "flat", {
    successfulPostCreateProof: true,
    mismatchedSuccessfulPostCreateNestedRef: true
  });
  const previous = process.env.OPENCLAW_FAKE_CALLS;
  process.env.OPENCLAW_FAKE_CALLS = callsPath;
  try {
...
test("OpenClaw tool smoke rejects an asymmetric post-create index-ref mismatch", () => {
  const dir = mkdtempSync(join(tmpdir(), "loo-openclaw-tool-smoke-post-create-mismatched-index-ref-"));
  const toolName = "loo_codex_start_thread_post_create_proof";
  const { bin, callsPath } = createFakeOpenClaw(dir, [toolName], "flat", {
    successfulPostCreateProof: true,
    mismatchedSuccessfulPostCreateIndexRef: true
  });
  const previous = process.env.OPENCLAW_FAKE_CALLS;
  process.env.OPENCLAW_FAKE_CALLS = callsPath;
  try {
    ...
    assert.equal(report.invocations[0]?.disposition, "expected_fail_closed");
    assert.equal(report.invocations[0]?.productEvidenceClaimed, false);
    assert.equal(report.invocations[0]?.blockers.includes("post_create_proof_index_source_ref_mismatch"), true);
  } finally {
    if (previous === undefined) delete process.env.OPENCLAW_FAKE_CALLS;
    else process.env.OPENCLAW_FAKE_CALLS = previous;
  }
...
test("OpenClaw tool smoke rejects inconsistent post-create action aliases", () => {
  const dir = mkdtempSync(join(tmpdir(), "loo-openclaw-tool-smoke-post-create-inconsistent-action-"));
  const toolName = "loo_codex_start_thread_post_create_proof";
  const { bin, callsPath } = createFakeOpenClaw(dir, [toolName], "flat", {
    successfulPostCreateProof: true,
    inconsistentSuccessfulPostCreateActionAlias: true
  });
  const previous = process.env.OPENCLAW_FAKE_CALLS;
  process.env.OPENCLAW_FAKE_CALLS = callsPath;
  try {
...
test("OpenClaw tool smoke preserves restricted-action evidence on unresolved post-create proof", () => {
  const dir = mkdtempSync(join(tmpdir(), "loo-openclaw-tool-smoke-post-create-unresolved-restricted-"));
  const toolName = "loo_codex_start_thread_post_create_proof";
  const { bin, callsPath } = createFakeOpenClaw(dir, [toolName], "flat", {
    currentProductFailClosedShapes: true,
