...
test("MCP tools expose default Codex roots and read-only SQLite probes", async () => {
  ...
  try {
    ...
    assert.ok(indexTool);
    const indexed = await indexTool.execute({}) as { indexedFiles: number; indexedThreads: number };
    ...
    assert.ok(probeTool);
    const probe = await probeTool.execute({ roots: [codexHome] }) as { stores: Array<{ path: string; supported: boolean }> };
