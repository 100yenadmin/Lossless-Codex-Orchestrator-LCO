...
test("canonical CLI help advertises lco while retaining loo as an executable alias", () => {
  ...
  assert.match(result.stdout, /lco qa-lab eva-idle-route .*\[--package-tarball path\]/);
...
test("loo search --help exits zero without querying the local index", () => {
  ...
  try {
    ...
    assert.match(result.stdout, /session-card search/i);
    assert.match(result.stdout, /title\/metadata/i);
    assert.match(result.stdout, /For content phrases, use `loo grep` or `loo expand-query`/);
...
test("loo hermes smoke --help exposes a local-only compatibility gate", () => {
  ...
  assert.match(result.stdout, /sixteen required Eva tools/i);
...
test("lco qa-lab eva-idle-route --help exposes the bounded candidate-only ladder", () => {
  ...
  assert.equal(result.status, 0, result.stderr || result.stdout);
  assert.match(result.stdout, /Usage:\n  lco qa-lab eva-idle-route .*\[--package-tarball path\].*\[--execute\] \[--strict\]/);
  assert.match(result.stdout, /--package-tarball path.*required with --execute/i);
