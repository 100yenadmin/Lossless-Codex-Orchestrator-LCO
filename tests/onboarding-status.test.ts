...
test("loo onboard status writes a public-safe first-run readiness artifact", () => {
  ...
  try {
    ...
    assert.ok(report.installRecovery.setupGuidance.some((item) => item.includes("tarball fallback")));
    ...
    assert.ok(report.nextSafeCommands.includes(report.installRecovery.tarballLookupCommand));
    assert.ok(report.nextSafeCommands.includes(report.installRecovery.globalInstallTarballFallbackCommand));
