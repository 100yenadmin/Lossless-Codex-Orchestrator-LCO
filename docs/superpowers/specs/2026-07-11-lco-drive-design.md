...
- `--objective <text>`
- `--surface cli|mcp|openclaw-gateway` (default `cli`)
...
The canonical MCP tool is `lco_drive`; `loo_drive` is the compatibility alias. It accepts equivalent snake-case fields. OpenClaw exposes the same generated facade rather than a separate implementation.
...
2. A deterministic drive plan with `review`, `plan`, `dry_run`, `confirm`, `live`, and `report` steps. Every step has an execute flag and state.
...
4. A controller matrix for CLI, MCP, OpenClaw, Codex, and Claude dry-run reachability.
