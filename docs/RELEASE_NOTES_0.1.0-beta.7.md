...
- Everything shipped in `0.1.0-beta.6`.
- Fixes OpenClaw plugin manifest version drift found during published package
  dogfood in #140.
- The published package dogfood installed
  `lossless-openclaw-orchestrator@0.1.0-beta.6`, but OpenClaw runtime inspect
  reported plugin metadata version `0.1.0-beta.1`.
- #141 and PR #142 aligned both OpenClaw plugin manifests with `package.json`
  and added a release contract test so future package versions cannot silently
  drift from runtime plugin metadata.
- This beta publishes that metadata fix as `0.1.0-beta.7`.
...
```bash
loo release status --claim-scope codex-read-search-expand-dry-run --evidence-dir /Volumes/LEXAR/Codex/lossless-openclaw-orchestrator/YYYY-MM-DD/release-status --candidate-sha <release-candidate-sha> --npm-publish-approval-evidence /Volumes/LEXAR/Codex/lossless-openclaw-orchestrator/YYYY-MM-DD/release-status/npm-approval.json --github-release-approval-evidence /Volumes/LEXAR/Codex/lossless-openclaw-orchestrator/YYYY-MM-DD/release-status/github-release-approval.json --github-ci-evidence /Volumes/LEXAR/Codex/lossless-openclaw-orchestrator/YYYY-MM-DD/release-status/codeql.json --strict
```
