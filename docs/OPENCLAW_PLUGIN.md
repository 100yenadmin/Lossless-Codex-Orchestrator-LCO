...
- `lco_codex_start_thread` is the approval-gated new-thread workflow. It is dry-run by default, requires the matching `approval_audit_id` for live `thread/start`, pins the thread to `approvalPolicy=never` plus a read-only sandbox, and reports `proof_state` so callers can distinguish `accepted_by_transport`, `started`, `completed`, `persisted`, and `unverified_pending`. An `unverified_pending` result is not durable execution or local-session persistence; callers must run the returned read-only `next_proof` packet before building follow-up claims.
