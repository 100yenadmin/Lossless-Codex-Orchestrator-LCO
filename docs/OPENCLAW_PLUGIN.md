...
- Dry-run output includes `params_hash` and message-bearing actions include `message_hash`; agents should echo those hashes before asking the user to approve live control.
- `loo_audit_tail` returns recent hashed audit records without raw prompt text.
