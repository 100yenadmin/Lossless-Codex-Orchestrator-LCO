...
for existing OpenClaw automation. The installed plugin exposes the canonical
`lco_*` tools; `loo_*` tool aliases remain maintained for at least two minor
releases.
...
- `lco_codex_start_thread` is the approval-gated new-thread workflow. It is dry-run by default, requires the matching `approval_audit_id` for live `thread/start`, and pins the thread to `approvalPolicy=never` plus a read-only sandbox. It reports `proof_state` so callers can distinguish `accepted_by_transport`, `started`, `completed`, `persisted`, and `unverified_pending`. An `unverified_pending` result is not durable execution or local-session persistence; callers must run the returned read-only `next_proof` packet before building follow-up claims.
- Live Codex resume/send/steer/interrupt also request `approvalPolicy=never` and a read-only, no-network sandbox instead of inheriting ambient permissions. Steer and interrupt rejoin the exact target on the same app-server connection, fail closed unless the resume response proves that posture is already active, and require `expected_turn_id`; interrupt binds it to Codex's current `turnId` field.
