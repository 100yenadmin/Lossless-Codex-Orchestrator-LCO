# Release Notes 0.1.0-beta.12

`lossless-openclaw-orchestrator` now includes the public-safe onboarding
readiness gate for the Codex-first working-app beta through the installed OpenClaw gateway.
This prerelease keeps the beta posture narrow: local Codex sessions indexing,
search, describe, bounded expansion, approval-gated Codex
control proof, post-action refresh, and public-safe reasoning from source refs.
...
- Allowed claim: Codex-first working-app proof through the installed OpenClaw
  gateway for local session recall, onboarding readiness, one approved live
  Codex action, post-action refresh, and safe reasoning from refs.
...
- `latest` remains pinned to `0.1.0-beta.4`; `beta` points at `0.1.0-beta.12`
