# Release Notes 0.1.0-beta.18

`lossless-openclaw-orchestrator` keeps the Codex-first working-app beta scope
for local Codex sessions and fixes the desktop proof-report release-gate handoff
wording. This prerelease does not broaden the public claim to generic GUI
mutation, Claude parity, cloud sync, unattended desktop takeover, or
enterprise/customer-ready security.

## Included

- #160 keeps the beta.17 proof-report behavior: a valid public-safe,
...
- The release-gate docs now distinguish the proof-report artifact directory
  from the desktop approval evidence file path.
...
- Invalid observations do not emit the desktop collaboration runtime proof
  marker.
- This is a desktop collaboration proof-report documentation hardening release,
  not a broad GUI automation release.

## Proof Boundary

- Allowed claim: Codex-first working-app beta through the installed OpenClaw
  gateway, with clearer desktop collaboration proof artifact handoff when that
  optional surface is claimed.
...
- `latest` remains pinned to `0.1.0-beta.4`; npm `beta` points at `0.1.0-beta.18`
...
`loo desktop proof-report --strict` against the public-safe observation. Use the
shared runtime-proof directory for `--runtime-proof-dir`; it must contain the
normal working-app runtime markers and
`desktop-collaboration-action-bound-v1-1.runtime-proof.json`. Copy the generated desktop runtime marker into that shared directory, and use the generated approval file for `--desktop-gui-approval-evidence`, for example
`/Volumes/LEXAR/Codex/lossless-openclaw-orchestrator/YYYY-MM-DD/proof-report/desktop-gui-approval.json`.

The marker `desktop-collaboration-action-bound-v1-1.runtime-proof.json` must
include an `action_hash` matching `desktop-gui-approval.json`.
