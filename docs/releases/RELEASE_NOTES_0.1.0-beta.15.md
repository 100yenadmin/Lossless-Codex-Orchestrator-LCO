# Release Notes 0.1.0-beta.15

`lossless-openclaw-orchestrator` keeps the Codex-first working-app beta scope
for local Codex sessions and hardens the claim-conditional desktop
...
- #160 hardens `loo release status --desktop-gui-required` so desktop GUI
  approval evidence must bind `actionHash` to the exact
  `desktopBackend`, `targetApp`, `targetWindow`, and `action` tuple emitted by
  `loo desktop proof-report`.
- This is a desktop collaboration gate hardening release, not a broad GUI
  automation release.
- A merely well-formed 64-character hash no longer satisfies the desktop GUI
  approval gate when desktop collaboration is claimed.
- Claim docs, VISION, and scorecards now describe the action-hash proof
  boundary explicitly.
...
- `latest` remains pinned to `0.1.0-beta.4`; `beta` points at
  `0.1.0-beta.15` if this candidate is published.
- npm `beta` points at `0.1.0-beta.15` if this candidate is published.
...
`desktop-collaboration-action-bound-v1-1.runtime-proof.json`, and
