# Release Notes 0.1.0-beta.11

`lossless-openclaw-orchestrator` now has Codex-first working-app proof for the
installed OpenClaw gateway path. This prerelease keeps the beta posture narrow:
...
- #158 proves one approved harmless live Codex send through the installed
  OpenClaw gateway after a matching dry-run approval audit id.
...
- Allowed claim: Codex-first working-app proof through the installed OpenClaw
  gateway for local session recall, one approved live Codex action, post-action
  refresh, and safe reasoning from refs.
...
- Release bundle and status checks do not publish to npm or create a GitHub
  Release; publishing is a separate explicit operation after gates pass.
- Release bundle and status checks do not create a GitHub Release.
- `latest` remains pinned to `0.1.0-beta.4`; `beta` points at `0.1.0-beta.11`.
