...
- Everything shipped in `0.1.0-beta.1`.
- A repair for isolated `lco-dogfood` OpenClaw gateway smoke runs where call-mode
  auth required explicit token forwarding.
- A backend loopback gateway call path for `loo openclaw tool-smoke`, using the
  documented OpenClaw backend client identity when an explicit gateway URL and
  token are supplied.
- Regression coverage proving gateway tokens are not written into public-safe
  smoke evidence.
