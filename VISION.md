...
- Use #326 to make the collaboration cockpit actionable without becoming mutating: `loo_codex_collaboration_next_steps` emits exact execute=false packets for watcher resume requests, Desktop coherence probes, and fallback-status checks, while approval/action lanes remain blocked until a separate proof gate exists.
