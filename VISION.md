...
- Keep Claude read/recall separate from parity: `indexClaudeSessionInventory` supports explicit redacted fixtures, and `indexClaudeSessions` / `lco index claude` can import local Claude Code JSONL into public-safe `claude_session:*` recall. Claude live control, GUI mutation, MCP control, hooks mutation, cloud sync, and parity remain future adapter work.
