...
The core Codex recall, M9 handoff paths, Codex Autonomy Cockpit, Eva Operating Picture P0 paths, release metadata, package/gateway setup proof, docs truth, release gates, public-safe scorecards, post-publish fresh npm `@latest` install proof, and stable `1.0.0` publication are no longer the main gap. The current release train is complete; #302/#304/#305/#306 close the stable `1.0.0` promotion, publication, and post-GA claim-validation gates. The next gap is 1.1 product polish: richer active-thread state, autonomous thread-management recommendations, and user-visible Desktop collaboration cockpit workflows without weakening 1.0 safety boundaries. `loo_codex_desktop_coherence` distinguishes `cli_visible`, `desktop_visible`, `desktop_refresh_required`, `desktop_restart_required`, and `unknown`; `loo_codex_desktop_fallback_status` reports the CUA-first and Peekaboo-secondary readiness path without performing live control or GUI mutation; `loo_codex_collaboration_cockpit` composes those structured reports with recent-session cards, inbox urgency, and watcher requests into one read-only active-lane summary; `loo_codex_runtime_desktop_visibility_status` summarizes which lanes have runtime Desktop visibility or action-bound proof coverage and which read-only `execute=false` probe remains next; `loo_codex_active_thread_state` classifies active threads as running, blocked, needs-nudge, stale, waiting, approval-needed, idle, or unknown from public-safe indexed, watcher, app-server, and visible-map signals.

The current target is:

- Keep the published npm `latest` package aligned with stable `1.0.0`, keep `beta` aligned with beta trains, and use `next` for future release candidates.
...
- Use #351 to give agents one read-only active-thread state report across cockpit lanes. It may report running, blocked, needs-nudge, stale, waiting, approval-needed, idle, or unknown state with reason codes, confidence, freshness, and source coverage, but it must not run live control, mutate Codex Desktop, capture screenshots, publish npm, or create GitHub releases.
...
- Use `loo_codex_active_thread_state` when the agent needs a compact answer to "which active Codex threads are running, blocked, stale, or need a nudge?" The report returns state counts, confidence, freshness, reason codes, source coverage, and false action flags without reading raw transcripts or mutating Codex.
...
| Active-thread state | Agent can classify running, blocked, needs-nudge, stale, waiting, approval-needed, idle, and unknown Codex lanes with confidence, reason codes, freshness, and source coverage without raw transcript reads or mutation | `loo_codex_active_thread_state` evidence |
