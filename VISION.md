...
The stable 1.0.0 package has shipped on npm `latest` and the GitHub Release is published. The current product/eval target is the post-GA Desktop visibility lane: prove or bound whether Codex CLI/direct-protocol/app-server actions are reflected in Codex Desktop without restart, then route CUA/Peekaboo fallback proof only if needed. Desktop parity [#307](https://github.com/100yenadmin/Lossless-Codex-Orchestrator-LCO/issues/307) added the coherence classifier; the active tracker is desktop fallback [#308](https://github.com/100yenadmin/Lossless-Codex-Orchestrator-LCO/issues/308), with the operating loop [#16](https://github.com/100yenadmin/Lossless-Codex-Orchestrator-LCO/issues/16).
...
- Prove or bound Codex Desktop live-refresh coherence through #307 using public-safe structured evidence only.
- Route #308 CUA/Peekaboo fallback only when #307 shows Desktop visibility is missing, stale, refresh-required, restart-required, or unknown; use `loo_codex_desktop_fallback_status` before claiming any fallback path is ready.
...
The sprint remains Codex-first, local-first, read-only-first, and public-safe by default. It does not claim full business truth, customer readiness, Claude Code parity, remote sync, generic GUI mutation, unattended desktop control, permission bypass, or enterprise/customer-ready security. npm `latest` promotion and GitHub Release creation are complete for 1.0.0, but Desktop-visible collaboration remains proof-gated behind #307/#308.
