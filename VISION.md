...
- Desktop GUI mutation claims require a backend-specific observation validated by `loo desktop proof-report` or `loo_desktop_proof_report`; the proof-report command itself must not perform the GUI action, and release approval `actionHash` must match the exact backend/app/window/action tuple.
