...
`loo doctor` and `loo desktop see cua-driver` report CUA Driver availability, the preferred MCP stdio launch command (`cua-driver mcp` unless `LOO_CUA_DRIVER_BIN` overrides it), permission status, limitations, and whether focus changed during a status-only observation. `loo_desktop_act` is dry-run-only in this beta and does not perform GUI actions.
